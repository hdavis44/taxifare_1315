...
def load_data_to_bq(data: pd.DataFrame, gcp_project: str, bq_dataset: str,
                    table: str, truncate: bool) -> None:
    """
    ...
    data.columns = [
        f"_{column}" if not str(column)[0].isalpha()
        and not str(column)[0] == "_" else str(column)
...
if __name__ == '__main__':
    full_table_name = f"{os.environ.get('GCP_PROJECT')}.{os.environ.get('BQ_DATASET')}.{'example'}"
    client = bigquery.Client()
